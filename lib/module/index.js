...
import { DeviceEventEmitter, NativeEventEmitter, NativeModules, Platform, PermissionsAndroid } from 'react-native';
...
export let AndroidAudioSourceId = /*#__PURE__*/function (AndroidAudioSourceId) {
...
export let AndroidOutputFormatId = /*#__PURE__*/function (AndroidOutputFormatId) {
...
export let AndroidAudioEncoderId = /*#__PURE__*/function (AndroidAudioEncoderId) {
...
export let AppleAVAudioSessionModeId = /*#__PURE__*/function (AppleAVAudioSessionModeId) {
...
export let AppleAVEncoderAudioQualityId = /*#__PURE__*/function (AppleAVEncoderAudioQualityId) {
...
export let AppleAVLinearPCMBitDepthId = /*#__PURE__*/function (AppleAVLinearPCMBitDepthId) {
...
export let EventId = /*#__PURE__*/function (EventId) {
...
export let RecStopCode = /*#__PURE__*/function (RecStopCode) {
...
export let PlayStopCode = /*#__PURE__*/function (PlayStopCode) {
...
export class Audio {
  ...
  async verifyAndroidPermissionsEnabled() {
    ...
    if (Platform.OS === 'android') {
      try {
        ...
        if (grants['android.permission.RECORD_AUDIO'] === PermissionsAndroid.RESULTS.GRANTED && (Platform.Version >= 33 || grants['android.permission.WRITE_EXTERNAL_STORAGE'] === PermissionsAndroid.RESULTS.GRANTED && grants['android.permission.READ_EXTERNAL_STORAGE'] === PermissionsAndroid.RESULTS.GRANTED)) {
          return true;
        } else {
  ...
  async verifyAndroidRecordAudioEnabled() {
    ...
    if (Platform.OS === 'android') {
      try {
        if (await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.RECORD_AUDIO)) {
          return true;
        } else {
  ...
  async verifyAndroidWriteExternalStorageEnabled() {
    ...
    if (Platform.OS === 'android') {
      try {
        // TODO: Android 13 (API 33) storage permissions should be handled differently / more completely...
        if (Platform.Version >= 33 || (await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE))) {
          return true;
        } else {
  ...
  async verifyAndroidReadExternalStorageEnabled() {
    ...
    if (Platform.OS === 'android') {
      try {
        // TODO: Android 13 (API 33) storage permissions should be handled differently / more completely...
        if (Platform.Version >= 33 || (await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE))) {
          return true;
        } else {
  ...
  async resolveAndValidateRecordingOptions(recordingOptions) {
    ...
    if (lcFileNameOrPath === undefined) {
      ...
      if (Platform.OS === 'android' && ro.androidAudioEncoderId === AndroidAudioEncoderId.LPCM && ro.androidOutputFormatId === AndroidOutputFormatId.WAV || Platform.OS === 'ios' && ro.appleAudioFormatId === AppleAudioFormatId.lpcm) {
  ...
  async startRecorder({
    ...
    const [err, res] = await to(RnAudio.startRecorder(recordingOptions));
    if (err) {
  ...
  async pauseRecorder() {
    ...
    const [err, res] = await to(RnAudio.pauseRecorder());
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      await this.resetRecorder();
  ...
  async resumeRecorder() {
    ...
    const [err, res] = await to(RnAudio.resumeRecorder());
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      await this.resetRecorder();
  ...
  async stopRecorder(silent) {
    ...
    const [err, res] = await to(RnAudio.stopRecorder());
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
  ...
  async startPlayer({
    ...
    const [err, res] = await to(RnAudio.startPlayer(fileNameOrPathOrURL, httpHeaders, playbackVolume));
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      await this.resetPlayer();
  ...
  async pausePlayer() {
    ...
    this.removePlayUpdateCallback();
    const [err, res] = await to(RnAudio.pausePlayer());
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      await this.resetPlayer();
  ...
  async resumePlayer() {
    ...
    const [err, res] = await to(RnAudio.resumePlayer());
    if (err) {
  ...
  async stopPlayer() {
    ...
    const [err, res] = await to(RnAudio.stopPlayer());
    if (err) {
      const errStr = funcName + '- ' + err;
      elog(errStr);
      await this.resetPlayer();
  ...
  async seekToPlayer(timeMs) {
    ...
    const [err, res] = await to(RnAudio.seekToPlayer(timeMs));
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      return Promise.reject(errStr);
  ...
  async setPlayerVolume(volume) {
    ...
    const [err, res] = await to(RnAudio.setPlayerVolume(volume));
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      return Promise.reject(errStr);
  ...
  async setSubscriptionDuration(sec) {
    ...
    ilog(funcName);
    const [err, res] = await to(RnAudio.setSubscriptionDuration(sec));
    if (err) {
      const errStr = funcName + ' - ' + err;
      elog(errStr);
      return Promise.reject(errStr);
